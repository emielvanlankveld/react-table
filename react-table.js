...
var ReactTablePagination = function (_Component) {
  ...
  function ReactTablePagination(props) {
    ...
    _this.filterPages = function (visiblePages, totalPages) {
      return visiblePages.filter(function (page) {
        return page <= totalPages;
      });
    };

    _this.getVisiblePages = function (page, total) {
      if (total < 6 || page < 3) {
        return _this.filterPages([1, 2, 3, 4, 5], total);
      } else if (page + 2 > total) {
        return _this.filterPages([total - 4, total - 3, total - 2, total - 1, total], total);
      } else {
        return _this.filterPages([page - 2, page - 1, page, page + 1, page + 2], total);
      }
    };
  ...
  createClass(ReactTablePagination, [{
    ...
    value: function updateCurrentRows(props) {
      if (typeof props.sortedData !== 'undefined' //use props.data for unfiltered (all) rows
      && typeof props.page !== 'undefined' && typeof props.pageSize !== 'undefined') {
    ...
    value: function render() {
      ...
      var pageNumber = this.props.page + 1;
      var visiblePages = this.getVisiblePages(this.props.page, this.props.pages);
      this.updateCurrentRows(this.props);

      return React__default.createElement(
        'div',
        { className: classnames(className, '-pagination'), style: this.props.style },
        this.rowCount && React__default.createElement(
          'div',
          { className: '-summary' },
          'Showing ',
          this.rowMin,
        ...
        React__default.createElement(
          ...
          visiblePages.map(function (page, index, array) {
            return React__default.createElement(
              ...
              {
                onClick: function onClick() {
                  return _this3.changePage(page - 1);
                },
                key: page,
